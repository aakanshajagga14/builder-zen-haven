...
function computeHazard(r: HardwareReading | null): number {
  ...
  const gyroMag = Math.sqrt(
    (r.gyro_x ?? 0) ** 2 + (r.gyro_y ?? 0) ** 2 + (r.gyro_z ?? 0) ** 2,
  );
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            POST your device payloads to /api/sensors/latest (JSON). This page
            polls every 3s and computes rockfall risk from rain, soil moisture,
            vibration and IMU motion.
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Button onClick={toggle}>
            {timerRef.current ? "Disconnect" : "Connect"}
          </Button>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Status:{" "}
              {connected || timerRef.current ? "Connected" : "Disconnected"}
            </p>
            <p className="text-xs text-muted-foreground">
          ...
          <Card key={s.key} className="p-4">
            ...
            {s.key === "soil-moisture" && (
              <p className="text-sm">
                Soil moisture wet:{" "}
                <span className="font-semibold">
                  {reading?.soil_moisture_wet ? "Yes" : "No"}
                </span>
              </p>
            )}
            {s.key === "sw420" && (
              <p className="text-sm">
                Vibration detected:{" "}
                <span className="font-semibold">
                  {reading?.vibration_detected ? "Yes" : "No"}
                </span>
              </p>
            )}
            {s.key === "mpu6050" && (
              <div className="grid grid-cols-3 gap-3 text-sm">
                <div>
                  <p className="text-muted-foreground">Accel X</p>
                  <p className="font-medium">{reading?.accel_x ?? "--"}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">Accel Y</p>
                  <p className="font-medium">{reading?.accel_y ?? "--"}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">Accel Z</p>
                  <p className="font-medium">{reading?.accel_z ?? "--"}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">Gyro X</p>
                  <p className="font-medium">{reading?.gyro_x ?? "--"}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">Gyro Y</p>
                  <p className="font-medium">{reading?.gyro_y ?? "--"}</p>
                </div>
                <div>
                  <p className="text-muted-foreground">Gyro Z</p>
                  <p className="font-medium">{reading?.gyro_z ?? "--"}</p>
                </div>
              </div>
            )}
            {s.key === "bme280" && (
              <div className="grid grid-cols-3 gap-3 text-sm">
                <div>
                  ...
                  <p className="font-medium">
                    {reading?.temperature_bme != null
                      ? `${reading.temperature_bme.toFixed(1)}°C`
                      : "--"}
                  </p>
                </div>
                <div>
      ...
      <Card className="p-4">
        ...
        <p className="text-sm text-muted-foreground">
          Your device POSTs JSON (accel/gyro/rain/moisture/vibration, etc.) to
          /api/sensors/latest. The app polls that endpoint and computes a risk
          score combining rain, moisture and vibration, plus IMU motion. This
          feeds the dashboard and alerts.
        </p>
      </Card>
    </div>
  );
